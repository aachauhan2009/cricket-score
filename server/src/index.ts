...
import {
  Team, Player, Match, MatchState, Innings, BallEvent,
...
/* ------------ Helpers ------------ */
const ch = (id: string) => `match:${id}`;
const ballsFromOvers = (o: number) => o * 6;
const maxOversPerBowler = (totalOvers: number) => Math.ceil((totalOvers || 6) / 5);
...
async function eligibleBatters(matchId: string, st: any, inn: any) {
  ...
  const outSet = new Set<string>(outsRows.map(r => String(r.playerId)));
  const onField = new Set<string>([String(st.strikerId || ""), String(st.nonStrikerId || "")].filter(Boolean));
  return all.filter(p => !outSet.has(String(p._id)) && !onField.has(String(p._id)));
}

async function wicketsAllOutThreshold(inn: any) {
  const count = await Player.countDocuments({ teamId: inn.battingTeamId });
  return Math.max(0, count - 1); // all-out when wickets == batters - 1
}

async function totalsByTeam(matchId: string) {
...
async function finishMatchNow(match: any, winnerTeamId: any | null, isTie: boolean) {
  ...
  await MatchResult.updateOne(
    { matchId: match._id },
    { $set: { winnerTeamId: winnerTeamId || undefined, loserTeamId: winnerTeamId ? (String(winnerTeamId) === String(match.teamAId) ? match.teamBId : match.teamAId) : undefined, isTie, isNoResult: false } },
    { upsert: true }
  );
  ...
  try {
    ...
    for (const pl of (p.players || [])) {
      ...
      await Player.findOneAndUpdate(
        { teamId: team._id, fullName: pl.fullName },
        { $set: { teamId: team._id, fullName: pl.fullName, role: pl.role, battingStyle: pl.battingStyle, bowlingStyle: pl.bowlingStyle } },
        { upsert: true, new: true }
      );
    }
    ...
    for (const g of (p.groups || [])) {
      const doc = await Group.findOneAndUpdate(
        { tournamentId: tournament?._id, name: g.name },
        { $set: { tournamentId: tournament?._id, name: g.name } },
        { upsert: true, new: true }
      );
    ...
    for (const gt of (p.groupTeams || [])) {
      ...
      if (!g || !t) continue;
      await GroupTeam.findOneAndUpdate(
        { groupId: g._id, teamId: t._id },
        { $set: { groupId: g._id, teamId: t._id } },
        { upsert: true, new: true }
      );
    }
...
app.get("/api/groups", async (req, res) => {
  ...
  const q: any = {}; if (tournamentId) q.tournamentId = tournamentId;
...
app.get("/api/matches/:id/chase-info", async (req, res) => {
  ...
  if (!match) return res.status(HTTP.NOT_FOUND).json({ error: "match not found" });

  const inns = await Innings.find({ matchId }).sort("createdAt").lean();
  if (inns.length < 2) return res.json({ active: false });
...
app.get("/api/tournaments/:id/standings", async (req, res) => {
  ...
  for (const g of groups) tables[String(g._id)] = {};
  ...
  for (const m of matches) {
    ...
    if (!gA || !gB || gA !== gB) continue; // only count if both in same group
    ...
    if (played) {
      ...
      if (res?.isTie) {
        aRow.tied += 1; bRow.tied += 1;
        aRow.points += 1; bRow.points += 1;
      } else if (res?.isNoResult) {
        aRow.noResult += 1; bRow.noResult += 1;
        aRow.points += 1; bRow.points += 1;
      } else if (res?.winnerTeamId) {
        const w = String(res.winnerTeamId);
        if (w === teamA) { aRow.won += 1; aRow.points += 2; bRow.lost += 1; }
        else if (w === teamB) { bRow.won += 1; bRow.points += 2; aRow.lost += 1; }
        ...
        if (a === b) { aRow.tied += 1; bRow.tied += 1; aRow.points += 1; bRow.points += 1; }
        else if (a > b) { aRow.won += 1; aRow.points += 2; bRow.lost += 1; }
        else { bRow.won += 1; bRow.points += 2; aRow.lost += 1; }
      }
    }
  }
  ...
  const out = groups.map(g => {
    const rows = Object.values(tables[String(g._id)]).map((r: any) => {
...
app.post("/api/matches/:id/set-bowler", authRequired, async (req, res) => {
  ...
  if (!st || !inn) return res.status(HTTP.NOT_FOUND).json({ error: "state/innings not found" });
...
app.post("/api/matches/:id/new-batter", authRequired, async (req, res) => {
  ...
  if (!st.waitingForNewBatter) return res.status(400).json({ error: "Not expecting a new batter" });

  const { playerId, end } = req.body as { playerId: string; end?: "striker" | "non-striker" };
  const whichEnd = end || st.waitingForNewBatterEnd || "striker";
  const p = await Player.findById(playerId);
  if (!p) return res.status(HTTP.NOT_FOUND).json({ error: "player not found" });
  ...
  if (whichEnd === "striker") {
    st.strikerId = p._id; st.striker = p.fullName;
  } else {
    st.nonStrikerId = p._id; st.nonStriker = p.fullName;
  }
...
app.get("/api/matches/:id/openers/options", async (req, res) => {
  const st = await MatchState.findOne({ matchId: req.params.id });
  if (!st) return res.status(HTTP.NOT_FOUND).json({ error: "state not found" });
  const inn = await Innings.findById(st.currentInningsId);
  if (!inn) return res.status(HTTP.NOT_FOUND).json({ error: "innings not found" });

  if (!st.waitingForOpeners) return res.json({ waiting: false });
...
app.post("/api/matches/:id/openers", authRequired, async (req, res) => {
  ...
  const inn = st ? await Innings.findById(st.currentInningsId) : null;
  if (!st || !inn) return res.status(HTTP.NOT_FOUND).json({ error: "state/innings not found" });
  if (!st.waitingForOpeners) return res.status(400).json({ error: "Not waiting for openers" });

  const { strikerId, nonStrikerId, bowlerId } = req.body as { strikerId: string; nonStrikerId: string; bowlerId?: string };
  ...
  if (!s || !ns) return res.status(HTTP.NOT_FOUND).json({ error: "player not found" });
  ...
  st.strikerId = s._id; st.striker = s.fullName;
  st.nonStrikerId = ns._id; st.nonStriker = ns.fullName;
  ...
  if (bowlerId) {
    ...
    st.bowlerId = b._id; st.bowler = b.fullName;
  } else {
    st.bowlerId = undefined as any; st.bowler = "";
  }
...
/* ------------ Innings progression ------------ */
async function maybeAdvanceInnings(matchId: string) {
  const st = await MatchState.findOne({ matchId });
  if (!st) return;
  const match = await Match.findById(matchId);
  if (!match) return;
  ...
  if (st.waitingForNewBatter) st.waitingForNewBatter = false;

  const count = await Innings.countDocuments({ matchId });

  if (count === 1) {
    // start 2nd innings
    const nextBat = inn.bowlingTeamId as mongoose.Types.ObjectId;
    const nextBowl = inn.battingTeamId as mongoose.Types.ObjectId;
    const newInn = await Innings.create({ matchId, battingTeamId: nextBat, bowlingTeamId: nextBowl });

    st.currentInningsId = newInn._id;
    st.runs = 0; st.wickets = 0; st.balls = 0;
    st.strikerId = undefined as any; st.striker = "";
    st.nonStrikerId = undefined as any; st.nonStriker = "";
    st.bowlerId = undefined as any; st.bowler = "";
  ...
  if (a === b) await finishMatchNow(match, null, true);
  else await finishMatchNow(match, a > b ? match.teamAId : match.teamBId, false);
}
...
app.post("/api/matches/:id/ball", authRequired, async (req, res) => {
  ...
  if (st.waitingForOpeners) return res.status(400).json({ error: "Pick second-innings openers first" });
  ...
  if (strikerId) st.strikerId = strikerId;
  if (nonStrikerId) st.nonStrikerId = nonStrikerId;
  if (bowlerId) st.bowlerId = bowlerId;

  if (!st.strikerId) return res.status(400).json({ error: "strikerId missing" });
  if (!st.bowlerId) return res.status(400).json({ error: "bowlerId missing" });

  if (!st.striker) st.striker = (await Player.findById(st.strikerId))?.fullName ?? "";
  if (!st.nonStriker) st.nonStriker = st.nonStrikerId ? ((await Player.findById(st.nonStrikerId))?.fullName ?? "") : "";
  if (!st.bowler) st.bowler = (await Player.findById(st.bowlerId))?.fullName ?? "";
  ...
  if (st.nextBallFreeHit && effWicket && !isRunout) effWicket = false;
  ...
  if (kind === "wide" || kind === "no-ball") inn.extras += penalty;
  if (kind === "bye" || kind === "leg-bye") inn.extras += runs;
  await inn.save();
  ...
  if (kind === "normal" || kind === "no-ball") {
    batInc.runs = batRuns;
    if (batRuns === 4) batInc.fours = 1;
    if (batRuns === 6) batInc.sixes = 1;
  }
  if (faced) batInc.ballsFaced = 1;
  await PlayerStats.updateOne(
    { matchId, playerId: st.strikerId },
    { $setOnInsert: { teamId: inn.battingTeamId }, $inc: batInc },
    { upsert: true }
  );
  // Bowling stats
  await PlayerStats.updateOne(
    { matchId, playerId: st.bowlerId },
    {
  ...
  if (effWicket) {
    if (isRunout) {
      ...
      if (outId) await PlayerStats.updateOne({ matchId, playerId: outId }, { $set: { isOut: true, howOut: "runout" } });
      st.waitingForNewBatter = true;
      st.waitingForNewBatterEnd = (outEnd === "non-striker" ? "non-striker" : "striker");
      if (outEnd === "non-striker") { st.nonStrikerId = undefined as any; st.nonStriker = ""; }
      else { st.strikerId = undefined as any; st.striker = ""; }
    } else {
  ...
  // Strike logic
  const swap = () => {
    const a = st.strikerId; st.strikerId = st.nonStrikerId; st.nonStrikerId = a;
    const an = st.striker; st.striker = st.nonStriker; st.nonStriker = an;
  };
  if (kind === "normal" && isLegal && (batRuns % 2 === 1)) swap();
  if ((kind === "bye" || kind === "leg-bye") && isLegal && (runs % 2 === 1)) swap();
  if (kind === "no-ball" && (batRuns % 2 === 1)) swap();
...
app.get("/api/matches/:id/scorecard", async (req, res) => {
  ...
  if (st?.strikerId) needed.add(String(st.strikerId));
  if (st?.nonStrikerId) needed.add(String(st.nonStrikerId));
  if (st?.bowlerId) needed.add(String(st.bowlerId));

  const ids = Array.from(new Set([...stats.map(s => String(s.playerId)), ...needed]));
  if (!ids.length) return res.json({});

  const players = await Player.find({ _id: { $in: ids } }).lean();
  const pMap = new Map<string, any>(); players.forEach(p => pMap.set(String(p._id), p));
  ...
  const byTeam: Record<string, { batting: any[]; bowling: any[] }> = {};
  const b2o = (b: number) => `${Math.floor((b || 0) / 6)}.${(b || 0) % 6}`;
  for (const s of merged) {
    const p = pMap.get(String(s.playerId)); if (!p) continue;
...
app.get("/api/stats/players", async (req, res) => {
  const { tournamentId, groupId, limit = 100 } = req.query as any;

  const matchFilter: any = {};
  if (tournamentId) matchFilter.tournamentId = tournamentId;
  if (groupId) matchFilter.groupId = groupId;
  ...
  const psMatch: any = {};
  if (matchIds.length) psMatch.matchId = { $in: matchIds };
